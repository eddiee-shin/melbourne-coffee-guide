...
### [2026-04-10] 위치 관리 시스템(Location Management) 추가
- **위치 관리 전용 테이블(locations) 신설**: 카페의 위치(지역)를 체계적으로 관리하기 위한 DB 구축.
- **관리자 UI 고도화**:
    - 카페 편집 시 위치 정보를 드롭다운(Select) 방식으로 선택하도록 개선하여 데이터 일관성 확보.
    - 위치 목록을 직접 추가하거나 삭제할 수 있는 관리 대시보드 섹션 추가.
- **로그인 연동**: 관리자 인증 성공 시에만 위치 관리 메뉴가 노출되도록 보안 강화.

### [2026-04-10] 메인 화면 지역 필터 동적 연동
- **위치 필터 자동화**: 홈페이지의 지역 선택창(Location Select)을 DB의 `locations` 테이블과 실시간 연동.
- **확장성 확보**: 이제 관리자 페이지에서 새로운 지역(예: Carlton)을 추가하면 소스 코드 수정 없이 메인 화면에 즉시 반영됨.
- **AI 분류 최적화**: 스크래퍼 프롬프트에 새로운 지역을 추가하여 AI 수집 단계부터 자동 분류가 가능하도록 개선.
